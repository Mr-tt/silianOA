...
import com.itheima.action.PrivilegeAction;
...
public class PrivilegeTest extends SpringUtils{
	@Test
	public void test1(){
//		PrivilegeDao pd = (PrivilegeDao) c.getBean("privilegeDao");
//		pd.bulidPTreeByRid(1L);
		
//		PrivilegeService ps = (PrivilegeService) c.getBean("privilegeService");
//		ps.bulidPTreeByRid(1l);
		UserService us = (UserService) c.getBean("userService");
		User user = us.findEntityById(2l);
		PrivilegeService ps = (PrivilegeService) c.getBean("privilegeService");
		System.err.println(user.getUid()+user.getUsername());
		Collection<Privilege> cc = ps.getFunctionByUid(user);
		System.err.println(cc.size());		
	}
	/**
	 * 测试PrivilegeAction的加载权限树的方法 ：showPrivilegeTreeAction
	 */
	@Test
	public void test(){
		PrivilegeAction pa = (PrivilegeAction) c.getBean("privilegeAction");
		pa.showPrivilegeTreeAction();
	}
